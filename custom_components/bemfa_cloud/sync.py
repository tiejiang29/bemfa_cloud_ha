...
class Sync(ABC):
    ...
    def _get_topic_suffix() -> TopicSuffix:
        """Return the Bemfa device type implied by this sync's HA domain.

        Subclasses return their default suffix here. The effective suffix
        used at runtime is `topic_suffix`, which honors a user-configured
        type override stored in `self._config[OPTIONS_DEVICE_TYPE]`.
        """
        raise NotImplementedError
    ...
    @property
    def topic_suffix(self) -> TopicSuffix:
        """Return the effective Bemfa topic suffix.

        Honors a manual type override stored under OPTIONS_DEVICE_TYPE.
        Falls back to the class-declared default suffix when no override
        is set or the override value is not in the allowed override list.
        """
        override = self._config.get(OPTIONS_DEVICE_TYPE) if self._config else None
        if override:
            try:
                suffix = TopicSuffix(override)
            except ValueError:
                suffix = None
            if suffix in OVERRIDABLE_TOPIC_SUFFIXES:
                return suffix
        return self._get_topic_suffix()
    ...
    def topic(self) -> str:
        ...
        This is the *effective* topic used for actual Bemfa communication
        (TCP subscribe / publish, HTTP topic creation). It honors any
        user-configured type override via `topic_suffix`.
        """
    ...
    def default_topic(self) -> str:
        """The topic computed with the class-default suffix (no override).

        Used as a *stable* key in `self._config` and option-flow dicts so
        that changing the type override does not orphan the stored config.
        The actual Bemfa topic used for communication is `topic`.
        """
        return (
            TOPIC_PREFIX
            + hashlib.md5(self._topic_stable_id().encode("utf-8")).hexdigest()[:12]
    ...
    def invalidate_topic(self) -> None:
        """Force `topic` to be recomputed on next access.

        Currently a no-op because `topic` is computed fresh every access
        (no caching). Kept for forward compatibility and to make the
    ...
    def generate_details_schema(self) -> dict[str, Any]:
        ...
        Adds a `device_type` selector that lets the user override the
        Bemfa device type for this entity. The default option is the
        type implied by the HA domain (the class-declared suffix), so
        existing behavior is preserved unless the user picks a different
        type.
        """
        ...
        from .const import OVERRIDABLE_TOPIC_SUFFIXES

        # Build (value, label) pairs for the dropdown. The first option is
        # always "auto" (= use the HA-domain-implied suffix); the rest are
        # the 7 Bemfa base types in a stable order.
        _TYPE_LABELS: dict[str, str] = {
            TopicSuffix.OUTLET: "001 " + "插座 / Outlet",
            TopicSuffix.LIGHT: "002 " + "灯 / Light",
            TopicSuffix.FAN: "003 " + "风扇 / Fan",
            TopicSuffix.SENSOR: "004 " + "传感器 / Sensor",
            TopicSuffix.CLIMATE: "005 " + "空调 / Climate",
            TopicSuffix.SWITCH: "006 " + "开关 / Switch",
            TopicSuffix.COVER: "009 " + "窗帘 / Cover",
        }
        default_suffix = self._get_topic_suffix()
        options = [
            SelectOptionDict(value="", label="自动 (按实体类型) / Auto"),
        ] + [
            SelectOptionDict(value=str(s), label=_TYPE_LABELS[s])
            for s in OVERRIDABLE_TOPIC_SUFFIXES
        ]
        current = self._config.get(OPTIONS_DEVICE_TYPE, "") if self._config else ""
        if current and current not in {opt["value"] for opt in options}:
            current = ""

        schema: dict[str, Any] = {
            vol.Required(OPTIONS_NAME, default=self._name): str,
            vol.Optional(
                OPTIONS_DEVICE_TYPE,
...
class ControllableSync(Sync):
    ...
    def resolve_msg(self, msg: Any):
        ...
        LOGGER.debug("Bemfa Cloud resolve_msg: entity=%s, msg=%r (type=%s)", self._entity_id, msg, type(msg).__name__)

        if self._resolve_json_msg(msg, state.attributes):
            LOGGER.debug("Bemfa Cloud resolve_msg: resolved via JSON path")
            return

        LOGGER.debug("Bemfa Cloud resolve_msg: JSON path failed, trying # separator path")
    ...
    def _resolve_json_msg(
        ...
        suffix = self.topic_suffix
        # --- Switch / Outlet / TV: simple on/off ---
        ...
        # --- Light (002) ---
        # When the source HA entity is a real `light.*`, use the light domain services
        # and forward brightness / color-temp / RGB when present.
        # When the source is a non-light entity (e.g. switch overridden to light),
        # gracefully degrade to turn_on / turn_off on the source domain.
        if suffix == TopicSuffix.LIGHT:
            src_domain = self._entity_id.split(".", 1)[0]
            if src_domain == "light":
                ...
                return True
            # graceful degradation: non-light entity overridden to light
            from homeassistant.const import SERVICE_TURN_OFF, SERVICE_TURN_ON
            if payload.get("on", True) is False:
                self._async_call_service(src_domain, SERVICE_TURN_OFF, {})
            else:
                self._async_call_service(src_domain, SERVICE_TURN_ON, {})
            return True

        # --- Fan (003) ---
        # Real fan entities use set_percentage; overridden entities fall back to on/off.
