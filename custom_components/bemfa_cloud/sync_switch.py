...
class Switch(ControllableSync):
    ...
    def collect_supported_syncs(cls, hass):
        ...
        for state in hass.states.async_all(cls._supported_domain()):
            if (
                state.entity_id.startswith(f"{SWITCH_DOMAIN}.")
                and state.attributes.get(ATTR_DEVICE_CLASS) == SwitchDeviceClass.OUTLET
            ):
                continue
...
class Outlet(Switch):
    """Sync a Home Assistant outlet switch to Bemfa outlet device."""

    @staticmethod
    def get_config_step_id() -> str:
        return "sync_config_outlet"

    @staticmethod
    def _get_topic_suffix() -> TopicSuffix:
        return TopicSuffix.OUTLET
    ...
    def collect_supported_syncs(cls, hass):
        return [
            ...
            if state.attributes.get(ATTR_DEVICE_CLASS) == SwitchDeviceClass.OUTLET
        ]


@SYNC_TYPES.register("tv")
class TV(Switch):
    """Sync a hass media player entity to Bemfa TV device."""

    @staticmethod
    def get_config_step_id() -> str:
        return "sync_config_tv"

    @staticmethod
    def _get_topic_suffix() -> TopicSuffix:
        return TopicSuffix.TV
    ...
    def resolve_msg(self, msg: Any):
        ...
        command_map = {
            "volup": SERVICE_VOLUME_UP,
            "volume_up": SERVICE_VOLUME_UP,
            "voldown": SERVICE_VOLUME_DOWN,
            "volume_down": SERVICE_VOLUME_DOWN,
            "chup": SERVICE_MEDIA_NEXT_TRACK,
            "next": SERVICE_MEDIA_NEXT_TRACK,
            "chdown": SERVICE_MEDIA_PREVIOUS_TRACK,
            "previous": SERVICE_MEDIA_PREVIOUS_TRACK,
        }
        service = command_map.get(str(command).lower())
        if service is not None:
            self._async_call_service(MEDIA_PLAYER_DOMAIN, service, {})
            return

        super().resolve_msg(msg)
